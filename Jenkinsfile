pipeline {
	...
	stages {
		...
		stage('Build with OPTION_CLIENT_SERVER=OFF') {
			steps {
				dir ("build") {
					sh "cmake -D OPTION_OFFICIAL_ONLY=ON -D OPTION_CLIENT_SERVER=OFF -D OPTION_AUTOMATED_TESTING=ON -D OPTION_TRACKEDITOR=ON -D CMAKE_BUILD_TYPE=Debug -D CMAKE_INSTALL_PREFIX=. -D CMAKE_PREFIX_PATH=. .."
					sh "cmake -D OPTION_OFFICIAL_ONLY=ON -D OPTION_CLIENT_SERVER=OFF -D OPTION_AUTOMATED_TESTING=ON -D OPTION_TRACKEDITOR=ON -D CMAKE_BUILD_TYPE=Debug -D CMAKE_INSTALL_PREFIX=. .."
					sh "make"
				}
			}
		}
		...
		stage('Build with OPTION_CLIENT_SERVER=ON') {
			steps {
				dir ("build") {
					sh "cmake -D OPTION_OFFICIAL_ONLY=ON -D OPTION_CLIENT_SERVER=ON -D OPTION_AUTOMATED_TESTING=ON -D OPTION_TRACKEDITOR=ON -D CMAKE_BUILD_TYPE=Debug -D CMAKE_INSTALL_PREFIX=. -D CMAKE_PREFIX_PATH=. .."
					sh "cmake -D OPTION_OFFICIAL_ONLY=ON -D OPTION_CLIENT_SERVER=ON -D OPTION_AUTOMATED_TESTING=ON -D OPTION_TRACKEDITOR=ON -D CMAKE_BUILD_TYPE=Debug -D CMAKE_INSTALL_PREFIX=. .."
					sh "make"
				}
			}
		}
